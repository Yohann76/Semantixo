<template>
  <div class="internal-link-analysis-form">
    <div class="analysis-form">
      <!-- URL à analyser -->
      <div class="form-group">
        <label for="url-input" class="form-label">URL du site à analyser :</label>
        <input 
          id="url-input"
        ...
        <small class="form-help">
          Entrez l'URL principale du site dont vous voulez analyser le maillage interne
        </small>
      ...
      <div class="form-actions">
        ...
        >
          ...
          {{ loading ? 'Analyse en cours...' : 'Analyser le maillage interne' }}
        </button>
      </div>
    </div>
  </div>
...
const analyzeInternalLinks = async () => {
  ...
  try {
    if (!isAuthenticated.value) {
      emit('error', 'Vous devez être connecté pour analyser un site')
      return
    }

    const headers = getAuthHeaders()
    
    console.log('🔗 [INTERNAL LINK FORM] Envoi analyse:', {
    ...
    if (response.ok) {
      console.log('✅ [INTERNAL LINK FORM] Analyse réussie:', {
        seoScore: data.data.seoScore,
        internalLinksCount: data.data.internalLinks?.length || 0
      })
      ...
      console.error('❌ [INTERNAL LINK FORM] Erreur analyse:', data)
    }
  } catch (err) {
    const errorMessage = `Erreur de connexion: ${err.message}`
    emit('error', errorMessage)
    console.error('❌ [INTERNAL LINK FORM] Erreur réseau:', err)
  } finally {
    loading.value = false
  }
}
